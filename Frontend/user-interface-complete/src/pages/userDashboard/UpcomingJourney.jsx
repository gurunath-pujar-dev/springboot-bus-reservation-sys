...
import { FaBus, FaTrash, FaUsers } from 'react-icons/fa';
...
export default function UpcomingJourney() {
  ...
  // State to track which bookings have passengers expanded
  ...
  async function fetchUpcoming() {
    setLoading(true);
    try {
      const res = await axios.get('http://localhost:8080/api/bookings/user/upcoming', {
  ...
  async function handleCancel(bookingId) {
    ...
    try {
      ...
      const { message, refundAmount, refundPercent } = res.data;
      ...
      toast.success(
        <div>
          ...
          <div>
            ...
            <span style={{ color: '#16a34a' }}>₹{refundAmount.toFixed(2)}</span>
          </div>
          <div>
            <strong>Refund Percent:</strong>{' '}
            <span style={{ color: '#16a34a' }}>{refundPercent}%</span>
          </div>
        </div>,
        {
          position: 'top-center',
      ...
      const errorMsg = err.response?.data?.message || 'Cancellation failed. Try again later.';
      setCancelMsg(errorMsg);
  ...
  return (
    <section>
      <h2 className="text-2xl font-semibold mb-4 text-blue-700">Upcoming Journey</h2>

      {loading ? (
        <p>Loading…</p>
      ) : bookings.length === 0 ? (
        <div className="text-center bg-gradient-to-r from-blue-50 to-blue-100 p-8 rounded-xl shadow-md border border-blue-200">
          <p className="text-gray-500 italic text-xl mb-3">No upcoming journeys found.</p>

          <p className="text-gray-700 text-base sm:text-lg mb-6">
            Ready to plan your next trip? Book your tickets now and start your journey! 🚍
          </p>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      className="flex items-center gap-1 bg-red-500 text-white px-4 py-2 rounded-lg font-bold hover:bg-red-600 transition"
                        ...
                        {booking.passengers.map((p) => (
                          ...
                          >
                            ...
                            <div className="text-sm text-gray-700 mt-1">
                              ID: <span className="font-medium">{p.passengerId}</span> |{' '}
